export function normalizeAlert(input) {
  ...
  const isUnresolvedToken = (s) => {
    if (!s) return false;
    const v = s.trim();
    // Unresolved MacroDroid tokens often look like [notif_title] or [notif_text]
    if ((v.startsWith('[') && v.endsWith(']')) || /\b(notif?|notf)[-_]?(title|text|ttle|txt)\b/i.test(v)) return true;
    return false;
  };
  let titleRaw = safe(input.title);
  let msgRaw = safe(input.message);
  let locRaw = safe(input.location);

  // If MacroDroid variables were not inserted properly, ignore them
  if (isUnresolvedToken(titleRaw)) titleRaw = '';
  if (isUnresolvedToken(msgRaw)) msgRaw = '';
  if (isUnresolvedToken(locRaw)) locRaw = '';
